...
void RunModel(structModel *model){
    ...
    for(int kTime = 1; kTime <= model->timeLen; kTime++){
        ...
        stepKPlus = kTime%2;
        #pragma omp parallel for num_threads(model->totalThreads) default(none) shared(model, lowerNeumannBC, rightNeumannBC, upperNeumannBC, leftNeumannBC, kTime, stepKMinus, stepKPlus, qoI)\
