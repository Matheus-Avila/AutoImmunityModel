...
void solve_ode(N_Vector y, int num_days, realtype **S, realtype **I, realtype **R, realtype **D, struct SIR_params params) {
    ...
    flag = CVodeInit(cvode_mem, SIR_PP_V5, 0, y);
...
static double model(realtype *x, int num_days) {
    ...
    realtype ti = x[2];
    realtype delta = x[3];
    realtype delta2 = x[11];
    realtype delta3 = x[12];
	...
	if(ti + delta + delta2 > num_days) {
        delta2 = num_days - (ti + delta);
		x[11] = delta2;
    }

	if(ti + delta + delta2 + delta3 > num_days) {
        delta3 = num_days - (ti + delta + delta2);
		x[12] = delta3;
    }
...
int main(int argc, char **argv) {
	...
	realtype l_bounds[] = {config->amax_l,  config->r_l,    ti1_l,          delta_l,        config->e_l,
                           config->theta_l, config->tau1_l, config->tau2_l, config->tau3_l, config->m_l, .7, delta_l, delta_l};
    ...
    realtype h_bounds[] = {config->amax_h,  config->r_h,    ti1_h,          delta_h,        config->e_h,
                           config->theta_h, config->tau1_h, config->tau2_h, config->tau3_h, config->m_h, 1, delta_h, delta_h};

    int num_par = 13;

	if(config->de_use_latin_hypercube) {
		sds command1 = sdsempty();
		command1 = sdscatprintf(command1, "python ./python_scripts/init_pop.py %d %d", config->pop_size, num_par);
		system(command1);
	}
	...
	//double x[] = {0.253094, 0.447193, 13.160316, 2.000032, 0.000000, 0.093024, 15.476657, 23.176447, 7.546812, 0.013089};
	
	//
	//E CONF:  0.12843294526804536
	//E DEATHS:  1.4083377954923413

	//double x[] = {2.70907856e-01, 0.2, 4, 10, 3.67707230e-04, 
//	                     8.07905931e-02, 1.20398872e+01, 1.87032508e+01, 1.03100444e+01, 1.62564638e-02, 0.5, 10};

    char *names[] = {"b", "r", "ti", "delta", "e", "not", "tau_1", "tau_2", "tau_3", "m", "r2", "delta2", "delta3"};
