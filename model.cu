...
float* EquationsLymphNode(structModel model, float* populationLN, int stepPos){
    ...
    float tCytoMigration = ( 1 - model.parametersModel.epsilonx ) * model.parametersModel.gammaT * (tCytoLN - model.tCytotoxicTissueVessels) * (float)(model.parametersModel.V_BV/model.parametersModel.V_LN);
